...
これは **今どう動いているか** の説明です。
構築手順は [06-github-actions-cicd.md](./06-github-actions-cicd.md)、最短入口は [11-cd-canonical-flow.md](./11-cd-canonical-flow.md) を参照してください。
...
- ここで大事なのは、`IMAGE_TAG` は **SSM Parameter Store の値ではない**ことです。
- `IMAGE_TAG=${github.sha}` は、GitHub Actions が SSM Run Command の中で EC2 に渡す環境変数です。
...
| 種類 | 例 | どこから来るか |
|---|---|---|
| SSM Parameter Store の値 | `DJANGO_SECRET_KEY`, `POSTGRES_HOST` | Terraform が登録し、EC2 が取得する |
| GitHub Actions から EC2 に渡す環境変数 | `IMAGE_TAG`, `DEPLOY_RUN_ID`, `DEPLOY_SHA` | workflow 実行時にその場で渡す |
...
- 現在は、まず `aws ssm wait command-executed` で command の完了を待ちます。
- その後に `aws ssm get-command-invocation` で最終 `Status` を確認します。
- 別 step の `Fetch SSM invocation logs` で、`StandardOutputContent` と `StandardErrorContent` を取得します。
- 状態の見方は次の通りです。
...
- GitHub Actions が `COMMAND_ID` を出す
- GitHub Actions が `DEPLOY_RUN_ID` と `DEPLOY_SHA` を EC2 に渡す
- EC2 が `.release/current` を更新する
- EC2 が `.release/last-run-id` を更新する
- EC2 が `.release/last-deploy-sha` を更新する

このため、`GitHub run -> SSM command -> EC2 release record` を 1 本で辿れます。
...
- 最短入口: [CD 正規フローの入口](./11-cd-canonical-flow.md)
- 失敗時の切り分け: [CD 切り分け表](./12-cd-troubleshooting.md)
- 実戦記録: [GitHub Actions デプロイ復旧の記録](./13-cd-recovery-log.md)
